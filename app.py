...
with aba_dash:
    ...
    if not df_cadastros.empty:
        ...
        elif filtro == "HidroVencido": df_resultado = hidro_vencido[colunas_dash].copy()
        elif filtro == "HidroProx": df_resultado = hidro_proximo[colunas_dash].copy()
        else: df_resultado = df_cadastros[colunas_dash].copy()

        # AJUSTE DE DATAS PARA O PADRÃO BRASILEIRO NO DASHBOARD
        for col_data in ["Próx. Recarga", "Próx. Teste"]:
            if col_data in df_resultado.columns:
                df_resultado[col_data] = pd.to_datetime(df_resultado[col_data]).dt.strftime('%d/%m/%Y')
